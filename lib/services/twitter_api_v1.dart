...
class TwitterApi {
  ...
  // This is the api key from the twitter developer page
  final String apiKey;

  /// This is the api key secret for twitter
  final String apiKeySecret;  // consumer secret v1

  /// This is the bearer token from twitter
  /// 
  final String token;

  final String tokenSecret; // for v2
  ...
  TwitterApi(this.apiKey, this.apiKeySecret, this.token, this.tokenSecret);
  ...
  /// 
  /// This is the where the information for this method comes from:
  ...
  Future<Response?> getTwitterRequest(String method, String url, 
    ...
    _oauth_timestamp = (DateTime.now().millisecondsSinceEpoch/1000).floor().toString();
    // Get the signature
    ...
    if(method.toUpperCase() == "GET")
    {
      ...
      // The timeout makes sure that if something goes wrong, the app doesn't 
      // hang forever
      response  = await get(
        Uri.https(
          "api.twitter.com", 
  ...
  /// Function to get a completed header for authorizing a request to twitter
  _getOauthHeader() {
    var dst = "OAuth ";
  ...
  _generateSignature(method, url, opt) {
    ...
    List<int> signingKey = utf8.encode(_percentEncode(apiKeySecret) + "&" + _percentEncode(tokenSecret));
  ...
  /// This function will percent encode [val] according to twitters rules 
  /// specified here:
  /// https://developer.twitter.com/en/docs/basics/authentication/guides/percent-encoding-parameters
  _percentEncode(String val) {
    ...
    for(var i = 0; i < bytes.length; i++) {
      ...
      if(!_percentEncodeValues.contains(char)) {
        ...
        // convert char to a string representing the hexadecimal of char
  ...
  _generateNonce() {
    ...
    var strippedNonce = encodedNonce.replaceAll(RegExp(r'[^a-zA-Z]'), '');
